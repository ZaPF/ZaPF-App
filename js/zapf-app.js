...
//		}
	  ...
	  if (Number(past) < Number(total)) {
		...
		$("span#next-ak").text('Als nächstes findet der Zeitblock "' + data.slots[next].name + '" statt : ' + String(time) + ' - ' + now.format('{24hr}:{mm}') + ' - ' + Date.create(data.slots[next].begin).format('{24hr}:{mm}'));
	  }
    },
    error: errorOccured,
  } );
